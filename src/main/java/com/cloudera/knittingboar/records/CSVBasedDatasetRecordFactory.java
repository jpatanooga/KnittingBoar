...
public class CSVBasedDatasetRecordFactory implements RecordFactory {
  ...
  // commas inside. Also, escaped quotes will not be unescaped. Good enough for
  // now.
  ...
  private static final Map<String,Class<? extends FeatureVectorEncoder>> TYPE_DICTIONARY = ImmutableMap
      .<String,Class<? extends FeatureVectorEncoder>> builder().put(
          "continuous", ContinuousValueEncoder.class).put("numeric",
          ContinuousValueEncoder.class).put("n", ContinuousValueEncoder.class)
      .put("word", StaticWordValueEncoder.class).put("w",
          StaticWordValueEncoder.class).put("text", TextValueEncoder.class)
   ...
   * @param typeMap
   *          A map describing the types of the predictor variables.
   */
  public CSVBasedDatasetRecordFactory(String targetName,
  ...
  public CSVBasedDatasetRecordFactory(String targetName, String idName,
      Map<String,String> typeMap) {
   ...
   * Note that any values of the target variable not present in this list will
   * be given the value of the last member of the list.
   * 
   * @param values
   ...
   * 
   * @param max
   *          The number of categories that will be excpeted. Once this many
   *          have been seen, all others will get the encoding max-1.
   */
        ...
        new Function<String,Integer>() {
          ...
          public Integer apply(String from) {
            ...
            Preconditions.checkArgument(r != null,
                "Can't find variable %s, only know about %s", from, vars);
            return r;
          }
        }));
    ...
    for (Integer predictor : predictors) {
      ...
      try {
        Preconditions.checkArgument(c != null,
            "Invalid type of variable %s,  wanted one of %s",
            typeMap.get(name), TYPE_DICTIONARY.keySet());
        Constructor<? extends FeatureVectorEncoder> constructor = c
            .getConstructor(String.class);
        Preconditions.checkArgument(constructor != null,
            "Can't find correct constructor for %s", typeMap.get(name));
  ...
  /**
   * Decodes a single line of csv data and records the target and predictor
   * variables in a record. As a side effect, features are added into the
   * featureVector. Returns the value of the target variable.
   * 
   * @param line
   *          The raw data.
   * @param featureVector
   ...
   * List<String> values = Lists.newArrayList(COMMA.split(line));
   * 
   * int targetValue = targetDictionary.intern(values.get(target)); if
   * (targetValue >= maxTargetValue) { targetValue = maxTargetValue - 1; }
   * 
   * for (Integer predictor : predictors) { String value; if (predictor >= 0) {
   * value = values.get(predictor); } else { value = null; }
   * predictorEncoders.get(predictor).addToVector(value, featureVector); }
   * return targetValue; }
   */
   ...
   * added into the featureVector. Returns the value of the target variable.
   * When used during classify against production data without target value, the
   * method will be called with returnTarget = false.
   * 
   * @param line
   *          The raw data.
   * @param featureVector
   ...
   *          processLine.
   * @param returnTarget
   *          whether process and return target value, -1 will be returned if
   *          false.
   * @return The value of the target variable.
   */
  public int processLine(CharSequence line, Vector featureVector,
